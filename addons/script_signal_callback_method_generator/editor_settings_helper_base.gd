...
func _convert_custom_bbcode(text:String) -> String:
	##フォントファイルやアイコンはキャッシュが必要
	##bbcodeじゃなくてコードから追加すればキャッシュは要らないが、bbcodeを自分で再実装しないといけないので断念
	##一々gitignoreを設定する必要性やキャッシュ作成時の動作等を考えた結果
	##機能を無効化
	##グローバルプラグイン機能が実装された場合、有効にすることも検討
	...
	text = text.replace("[code]", "[color=" + code_color + "][bgcolor=" + code_bg_color + "]")
	#text = text.replace("[code]", "[color=" + code_color + "][bgcolor=" + code_bg_color + "][font=" + CODE_FONT_PATH + "]")
	text = text.replace("[/code]", "[/bgcolor][/color]")

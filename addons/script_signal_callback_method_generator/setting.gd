...
func _add_settings_initialized() -> void:
	...
	set_description("line_space", "en", "The line spacing between this function and the one above it during generation when "+ code_block("action_mode") + " is " + code_block("ActionMode.GENERATE") + ".")
	set_description("line_space", "ja", code_block("action_mode") + "が" + code_block("ActionMode.GENERATE") + "の時、生成時の上の関数との行間隔。")
	...
	set_description("select_initial_text", "en", "When "+ code_block("action_mode") + " is " + code_block("ActionMode.GENERATE") + "," + code_block("initial_text") + " is automatically selected during generation.")
	set_description("select_initial_text", "ja", code_block("action_mode") + "が" + code_block("ActionMode.GENERATE") + "の時、生成時に自動で" + code_block("initial_text") + "を選択する。")
	
	add_setting("create_connect", true, TYPE_BOOL)
	set_description("create_connect", "en", "Create a `connect()` function. [br][b]Note[/b]: This setting works regardless of the value of " + code_block("action_mode") +".")
	set_description("create_connect", "ja", "connect()を作成する。[br][b]Note[/b]: " + code_block("action_mode") + "が何の場合でもこの設定は機能します。")
...
For information on the generation position, see " + code_block("action_generate_mode") )
...
生成位置については"+ code_block("action_generate_mode") + "を参照")
...
"When " + code_block("action_mode") + "is set to " + code_block("ActionMode.GENERATE") + ", this determines where the syntax is generated.
...
code_block("action_mode") + "が" + code_block("ActionMode.GENERATE") + "の時、どこに構文を生成するか。
...
[b]Next[/b]: 次にインデントが無い位置に構文を生成します。")
	...
	set_description("force_generate", "en", "When "+ code_block("action_mode") + " is " + code_block("ActionMode.GENERATE") +", the function will be created even if one with that name already exists.")
	set_description("force_generate", "ja", code_block("action_mode") + "が"+ code_block("ActionMode.GENERATE") + "の時、すでにその名前の関数がある場合でも作成する。")
